    ...
    <Tooltip.Root>
      <Tooltip.Trigger>
        <Card.Root 
          ...
          style={borderStyle()}
        >
          <Card.Content class="p-0 flex items-center justify-center gap-1.5 text-card-foreground">
      ...
      <Tooltip.Content side="top" class="max-w-[240px]">
        {#if tooltipData()!.header}
          <div class="font-semibold mb-1 text-foreground">{tooltipData()!.header}</div>
        {/if}
        {#if tooltipData()!.body}
          <div class="text-sm mb-1 text-foreground">{tooltipData()!.body}</div>
        {/if}
        {#if tooltipData()!.footer}
          <div class="text-xs italic text-muted-foreground">{tooltipData()!.footer}</div>
        {/if}
      </Tooltip.Content>
    </Tooltip.Root>
  {:else}
