 ...
 * @returns {Uint8Array}
 */
export function generate_clifford_attractor(width, height, is_dark) {
    const ret = wasm.generate_clifford_attractor(width, height, is_dark);
 ...
 * @returns {Uint8Array}
 */
export function generate_henon_attractor(width, height, is_dark, seed) {
    const ret = wasm.generate_henon_attractor(width, height, is_dark, seed);
    var v1 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
    wasm.__wbindgen_free(ret[0], ret[1] * 1, 1);
    return v1;
}
 ...
 * @returns {number}
 */
export function julia_iteration(cx, cy, zx, zy, max_iter) {
    const ret = wasm.julia_iteration(cx, cy, zx, zy, max_iter);
    return ret >>> 0;
}
